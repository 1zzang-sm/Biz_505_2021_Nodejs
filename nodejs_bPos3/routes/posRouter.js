...
/**
 * promise
 * 	지금까지 사용된 JS code는 대부분이 비동기 방식이고
 * 	코드(함수)들의 순서가 보장되지 않았다.
 * 	순서대로 작성된 코드가 지 맘대로 실행이 되기 때문에
 * 	그 부분을 해결하기 위해서 call back이라는 개념으로 코딩을 했다.
 * 
 ...
 * 		})
 * 	})
 * JS에서 이러한 상황을 Callback Hell 이라 부른다.
 * 
 * async await 
 * 
 */ 
router.get('/order/:table_id/input/:menu_id', async(req, res) =>{
	...
	// await가 부착된 함수는 
	// 함수의 실행이 완료되고 menu변수에 결과값이 담기기 전에는
	// 다음 코드로 진행이 안된다.
...
router.get("/paycomplete/:table_id", (req, res)=>{
	const table_id = req.params.table_id
	// 주문서에 결제가 완료된 표식으로 to_pay column에 문자열 P 업데이트
