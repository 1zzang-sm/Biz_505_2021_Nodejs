...
router.get("/detail", (req, res) => {
    // list에서 게시물을 클릭했을때 
    // 게시물의 id(b_id)값을
    // queryString으로 가지고 여기에 도달한다.
    const b_id = req.query.b_id;
    
    // PK를 기준으로 한개의 데이터를 추출하라
...
router.get("/delete", (req, res)=>{
    ...
    // 데이터를 삭제하라
    .destroy({
        // b_id column 의 값이 변수 b_id에 담긴 값과 같으면
        where: { b_id },
    })
    .then(()=>{
        res.redirect("/")
