...
const add_order_list = (order_list) => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
	...
	// button 클릭되었을때 결제처리를 수행하려고 했다.
	// button box포함하여 button을 동적으로 생성을 했다.
	// 동적으로 생성된 tag들은 자체적으로 event를 수신하지 못한다.
	// 아래의 event handler는 div의 button_box가 만들어지기 전에
	// 선언되고 OS에게 알려진 코드이다.
	// div.button_box가 아직 만들어지지 않은 상태에서
	// 선언된 event handler는 OS가 무시해 버린다.
	if(pay_box) {
	...
	// 처음에 아예 전체 HTML문서 자체에 click event를 설정해 둔다.
	...
	document.addEventListener("click", (e)=>{
		...
		if(button.tagName === "BUTTON") {
			...
			document.querySelector("span.pay_total").innerText 
				= order_pay_total
		}
	})
	
	// x button을 클릭하여 모달창 닫기
	document.querySelector("div.close").addEventListener("click", (e)=>{
